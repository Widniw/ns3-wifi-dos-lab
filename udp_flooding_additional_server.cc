...
int main(int argc, char *argv[])
{
  ...
  // Create stations, AP and attacker
  NodeContainer wifiStaNodes;
  wifiStaNodes.Create(nWifi);
  NodeContainer wifiApNode;
  wifiApNode.Create(1);
  NodeContainer wifiAttackerNodes;
  wifiAttackerNodes.Create(nAttacker);
  ...
  attackerDevices = wifi.Install(phy, mac, wifiAttackerNodes);
  ...
  mobility.Install(wifiAttackerNodes);
  ...
  stack.Install(wifiAttackerNodes);
  ...
  Ipv4InterfaceContainer attackerNodeInterface;
  Ipv4InterfaceContainer serverInterface;


  staNodeInterface = address.Assign(staDevice);
  apNodeInterface = address.Assign(apDevice);
  attackerNodeInterface = address.Assign(attackerDevices);
  serverInterface = address.Assign(serverDevice);

  // Install applications (traffic generators)
  ApplicationContainer sourceApplications, sinkApplications, attackerApplication, sinkattackerApplication;
  ...
  for (uint32_t index = 0; index < nWifi; ++index) // Loop over all stations (which transmit to the AP)
  {
    ...
    onOffHelper.SetConstantRate(DataRate(150e6), 1472);            // Set data rate (150 Mb/s divided by no. of transmitting stations) and packet size [B]
  ...
  // Simulator::Schedule(Seconds(1.0), &UdpFlood, wifiAttackerNode.Get(0), apAddress, 12345);
